                          ...
                          size_t K, size_t N)
{
    ...
    // Show printing behavior
    // if (threadIdx.x == 0 && threadIdx.y == 0)
    // {
    //     printf("**********************************************************************\n");
    //     printf("Hello from thread (%d, %d) in block (%d, %d) inside the matmul kernel!\n", threadIdx.x, threadIdx.y, blockIdx.x, blockIdx.y);
    //     printf("**********************************************************************\n");
    // }
...
{
    // Problem size
    const size_t M{4096}, K{4096}, N{4096};
    ...
    // Block CPU execution until specified event is recorded
    ...
    std::cout << "Duration (CUDA event API) = " << milliseconds << "ms" << std::endl;
    std::cout << "Duration (C++ perf timers) = " << ms.count() << "ms" << std::endl;
